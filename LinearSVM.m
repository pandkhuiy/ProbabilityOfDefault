function [PCC, AUC, PG,BS, KS, H] = LinearSVM(X,y)
...
KSvalue_vector = zeros(P*2,1);
 ...
 for  l = 1:P
  ...
  [C_optimal_PCC, C_optimal_AUC, C_optimal_PG, C_optimal_BS, C_optimal_H, C_optimal_KS] =  optimizeLinearSVM(X1, Y1);
  
  % Now fit a SVM with the optimal hyperparameter for each performance
  % measure
  trainedSVMModelPCC = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_PCC,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelKS = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_KS,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelAUC = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_AUC,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelPG = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_PG,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelH = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_H,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelBS = fitcsvm(X1,Y1,'BoxConstraint',C_optimal_BS,'KernelFunction','linear','ClassNames',[0,1]);
  ...
  % Determine the optimal penalty constant C with k fold cross validation
  % with k = 5 on the training data.
  [C_optimal_PCC, C_optimal_AUC, C_optimal_PG, C_optimal_BS, C_optimal_H, C_optimal_KS] =  optimizeLinearSVM(X2, Y2);
  
  % Now fit a SVM with the optimal hyperparameter for each performance
  % measure
  trainedSVMModelPCC = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_PCC,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelKS = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_KS,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelAUC = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_AUC,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelPG = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_PG,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelH = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_H,'KernelFunction','linear','ClassNames',[0,1]);
  trainedSVMModelBS = fitcsvm(X2,Y2,'BoxConstraint',C_optimal_BS,'KernelFunction','linear','ClassNames',[0,1]);
