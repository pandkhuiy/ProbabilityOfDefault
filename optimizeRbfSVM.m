function [C_optimal_PCC, C_optimal_AUC, C_optimal_PG, C_optimal_BS, C_optimal_H, C_optimal_KS,...
    S_optimal_PCC, S_optimal_AUC, S_optimal_PG, S_optimal_BS, S_optimal_H, S_optimal_KS] =  optimizeRbfSVM(Xtrain, Ytrain, useAdaSyn)
...
for j = 1:k
  ...
  if useAdaSyn == 1
    ...
    trainedSVMModel = fitcsvm([XAdaSyn;Xtrain(TrainingIndices(:,j),:)],[double(yAda);Ytrain(TrainingIndices(:,j))],'BoxConstraint',C(c),'KernelFunction','rbf','KernelScale',S(s),'ClassNames',[0,1]);
