function [PCC_KS_AUC_PG_H_BS_FNNnewMW, PCC_KS_AUC_PG_H_BS_BaggingFNNnewMW] = FNNnewMW(X1,X2,y1,y2)
...
PCC_KS_AUC_PG_H_BS_FNNnewMW = [PCC, KS, AUC, PG, H, BS];
 
%% Do bagging on FNNs with previously obtained optimal hyperparameters.
...
if useAdaSyn == 1
    ...
    for numOfBags = 1:length(NumberBagsVector)
        
        PCCm = zeros(size(Xtest4,1),numOfBags);
        KSm  = zeros(size(Xtest4,1),numOfBags);
        AUCm = zeros(size(Xtest4,1),numOfBags);
        PGm  = zeros(size(Xtest4,1),numOfBags);
        Hm   = zeros(size(Xtest4,1),numOfBags);
        BSm  = zeros(size(Xtest4,1),numOfBags);

        for b=1:NumberBagsVector(numOfBags)
            TrainMatrix=[[XAdaSyn;Xtrain123],[double(yAda);ytrain123]];
        ...
        % values.
        PCC =  sum( (Ytest4 == YhatPCC) )/numel(Ytest4);
        PerformanceMeasuresMatrix(numOfBags,1) = PCC;

        
        prior1 = mean([double(yAda);ytrain123]); prior0 = 1 - prior1;

        [AUC,~, ~ ] = computeAUC_PGindex_Hvalue(Ytest4, tempAUC, prior1, prior0);
        PerformanceMeasuresMatrix(numOfBags,3) = AUC;
        

        [~,PG_index, ~ ] = computeAUC_PGindex_Hvalue(Ytest4, tempPG, prior1, prior0);
        PerformanceMeasuresMatrix(numOfBags,5) = PG_index;
        

        [~,~, H_measure ] = computeAUC_PGindex_Hvalue(Ytest4, tempH, prior1, prior0);
        PerformanceMeasuresMatrix(numOfBags,4) = H_measure ;


        BScore = mean( (tempBS - Ytest4).^2);
        PerformanceMeasuresMatrix(numOfBags,6) = BScore;

        KS_value = computeKSvalue(Ytest4,tempKS);
        PerformanceMeasuresMatrix(numOfBags,2) = KS_value ;
    
    end
    
    PCC_vector(1) = max( PerformanceMeasuresMatrix(:,1));
    KSvalue_vector(1) = max( PerformanceMeasuresMatrix(:,2));
    AUC_vector(1) = max( PerformanceMeasuresMatrix(:,3));
    Hmeasure_vector(1) = max( PerformanceMeasuresMatrix(:,4));
    PGini_vector(1) = max( PerformanceMeasuresMatrix(:,5));
    BScore_vector(1) = min( PerformanceMeasuresMatrix(:,6));
    ...
    for numOfBags = 1:length(NumberBagsVector)
        ...
        for b=1:NumberBagsVector(numOfBags)
            TrainMatrix=[Xtrain123,ytrain123];
            Lengthdata=size(TrainMatrix,1);
            temp1=ceil(Lengthdata*rand(Lengthdata,1));
            bsdata=TrainMatrix(temp1,:);
            bsx=bsdata(:,1:end-1);
...
PCC_KS_AUC_PG_H_BS_BaggingFNNnewMW = [PCC, KS, AUC,PG,H, BS];
