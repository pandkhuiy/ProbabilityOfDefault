function [Prune_and_MinLeafSize_optimal_PCC, Prune_and_MinLeafSize_optimal_AUC, Prune_and_MinLeafSize_optimal_PG, Prune_and_MinLeafSize_optimal_BS,...
          Prune_and_MinLeafSize_optimal_H, Prune_and_MinLeafSize_optimal_KS] =  optimizeDT(Xtrain, Ytrain, useAdaSyn)
...
k = 5;
    ...
    for j = 1:k
      ...
      Tree = fitctree([XAdaSyn;Xtrain(TrainingIndices(:,j),:)],[double(yAda);Ytrain(TrainingIndices(:,j))],'Prune', Prune{l} ,'MinLeafSize', round(MinLeafSizeVector(h)*size([XAdaSyn;Xtrain(TrainingIndices(:,j),:)],1)) );
